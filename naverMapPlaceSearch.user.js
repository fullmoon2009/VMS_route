...
(function () {
  ...
  function log(...a) { if (DEBUG_LOG) console.log('[place-inject]', ...a); }
  function warn(...a) { console.warn('[place-inject]', ...a); }
  ...
  function deepFindMapInObject(root, depthLimit) {
    ...
    function rec(obj, depth) {
      ...
      const priority = ['map','naverMap','nMap','nm','mapObj','mapInstance','oMap','m'];
  ...
  async function proxyPlaceSearch(query) {
    const url = `${PROXY_BASE}${PROXY_ENDPOINT}?q=${encodeURIComponent(query)}&display=${DISPLAY_N}`;
  ...
  function forceMoveMap(map, latlng, label) {
    ...
    try { map.setCenter(latlng); } catch (e) { warn('setCenter err', e); }
    try { map.panTo(latlng); } catch (e) { warn('panTo err', e); }
  ...
  function mapxyToLatLng(mapx, mapy, cb) {
    ...
    if (
      ...
      log('mapxyToLatLng: using WGS84*1e7', { mapx, mapy, lat: lat1, lng: lng1 });
      cb(ll);
      return;
    }

    if (!window.naver?.maps?.TransCoord || !window.naver?.maps?.Point || !window.naver?.maps?.LatLng) {
    ...
    try {
      ...
      if (ret && typeof ret.lat === 'function' && typeof ret.lng === 'function') {
        fired = true;
        cb(ret);
        return;
      }
      ...
      setTimeout(() => {
        if (!fired) warn('mapxyToLatLng: TM128 callback not fired (SDK variant?)');
  ...
  function renderResults(items, onPick) {
    if (!items || !items.length) { setListHTML(''); return; }
    ...
    const rows = items.map((it, idx) => {
      ...
      const my = it.mapy;
      return `
        <div data-idx="${idx}" style="padding:8px;border-radius:8px;border:1px solid rgba(255,255,255,0.10);margin-bottom:6px;cursor:pointer;background: rgba(255,255,255,0.04);">
  ...
  async function doSearchAndMove(query) {
    ...
    renderResults(items, (it, idx) => {
      const title = stripHtml(it.title) || it.roadAddress || it.address || query;
      ...
      mapxyToLatLng(mapx, mapy, latlng => {
        if (!latlng) { setMsg('좌표 변환 실패'); return; }
  ...
  async function appendWhenBodyReady(el) {
    for (let i = 0; i < 200; i++) {
      if (document.body) {
  ...
  function ensureOverlayUI() {
    ...
    wrap.innerHTML = `
      ...
      <div id="__list__" style="margin-top:8px;max-height:260px;overflow:auto;border:1px solid rgba(255,255,255,0.12);border-radius:8px;padding:6px;display:none;"></div>
    ...
    const init = () => {
      ...
      document.getElementById('__ping__')?.addEventListener('click', async () => {
        setMsg('map 확보 중');
        const map = await ensureMapCaptured();
        if (!map) { setMsg('map 캡처 실패'); return; }
        try {
          const c = map.getCenter();
          setMsg(`map OK (center: ${c.y.toFixed(6)}, ${c.x.toFixed(6)} / zoom: ${map.getZoom()})`);
        } catch {
          setMsg('map OK');
        }
      });
